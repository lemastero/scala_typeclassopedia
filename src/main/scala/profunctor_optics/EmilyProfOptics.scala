...
trait Choice[=>:[_, _]] extends Profunctor[=>:] {
  ...
  def right[A, B, C]: A =>: B => (Either[C, A] =>: Either[C, B]) =
    dimap[Either[C, A], Either[C, B], Either[A, C], Either[B, C]](
      _.swap,
      _.swap
    ) compose left[A, B, C]
}
...
trait Magnified[=>:[_, _]] extends Strong[=>:] with Choice[=>:] {
  def maginfyLS[A, B, C, D]
      : (A =>: B) => ((C, Either[A, D]) =>: (C, Either[B, D])) =
    second compose left
  def maginfyRS[A, B, C, D]
      : (A =>: B) => ((C, Either[D, A]) =>: (C, Either[D, B])) =
    second compose right
  def maginfyLF[A, B, C, D]
      : (A =>: B) => ((Either[A, D], C) =>: (Either[B, D], C)) =
    first compose left
  def maginfyRF[A, B, C, D]
      : (A =>: B) => ((Either[D, A], C) =>: (Either[D, B], C)) =
    first compose right
}
...
trait Windowed[=>:[_, _]] extends Choice[=>:] with Glassed[=>:] {
  def windowedRGL[A, B, U, V, T]
      : (A =>: B) => (Either[V, (T, U => A)] =>: Either[V, (T, U => B)]) =
    right compose glassedL
  def windowedRGR[A, B, U, V, T]
      : (A =>: B) => (Either[V, (U => A, T)] =>: Either[V, (U => B, T)]) =
    right compose glassedR
  def windowedLGL[A, B, U, V, T]
      : (A =>: B) => (Either[(U => A, T), V] =>: Either[(U => B, T), V]) =
    left compose glassedR
  def windowedLGR[A, B, U, V, T]
      : (A =>: B) => (Either[(U => A, T), V] =>: Either[(U => B, T), V]) =
    left compose glassedR
}
