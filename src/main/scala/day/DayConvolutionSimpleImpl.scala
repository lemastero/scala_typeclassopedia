...
object DayConvolutionSimpleImpl {
  ...
  trait Day[F[_], G[_], A] { self =>
    ...
    def trans1[H[_]](nat: NaturalTransf[F, H]): Day[H, G, A] = new Day[H, G, A] {
      type X = self.X
      type Y = self.Y
    ...
    def trans2[H[_]](nat: NaturalTransf[G, H]): Day[F, H, A] = new Day[F, H, A] {
      type X = self.X
      type Y = self.Y
      val fx: F[X] = self.fx
  ...
  object Day {
    ...
    def intro2[F[_], A](fa: F[A]): Day[F, Id, A] = new Day[F, Id, A] {
      ...
      type Y = Unit
      val fx: F[X] = fa
      val gy: Id[Y] = Id(value = ())
      def xya: (X, Y) => A = (a, _) => a
    }

    /** Collapse to second type constructor if first one is Identity */
    def elim1[F[_], A](d: Day[Id, F, A])(implicit FunF: Functor[F]): F[A] = FunF.map(d.gy)(d.xya(d.fx.value, _))

    /** Collapse to first type constructor if second one is Identity */
    def elim2[F[_], A](d: Day[F, Id, A])(implicit FunF: Functor[F]): F[A] = FunF.map(d.fx)(d.xya(_, d.gy.value))
