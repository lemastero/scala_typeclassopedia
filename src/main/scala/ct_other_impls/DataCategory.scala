...
object DataCategory {
  ...
  val TypesAndPureOneArgumentFunctionsCategory: Category[Function1] =
  ...
  def oppositeCategory[K[_, _]](implicit
      CC: Category[K]
  ): Category[Op[K, *, *]] =
    new Category[Op[K, *, *]] {
      def source[A, B](f: Op[K, A, B]): TheObject[Op[K, *, *], A] =
        Op(CC.target(f.unOp))
      def target[A, B](f: Op[K, A, B]): TheObject[Op[K, *, *], B] =
        Op(CC.source(f.unOp))
      def compose[A, B, C](f: Op[K, B, C], g: Op[K, A, B]): Op[K, A, C] =
        Op(CC.compose(g.unOp, f.unOp))
    }
