...
object FunctorInstances {
  val listFunctor1: Functor[List] =
    new Functor[List]() { // TODO add custom impl of List and move this guy there
      def map[A, B](fa: List[A])(ab: A => B): List[B] =
        fa match {
          case Nil          => Nil
  ...
  val leftTupleFunctor: Functor[TupleLeftInt] = new Functor[TupleLeftInt] {
    def map[A, B](fa: TupleLeftInt[A])(f: A => B): TupleLeftInt[B] =
      (f(fa._1), fa._2)
  }

  type EitherRightInt[X] = Either[Throwable, X] // map
  val eitherRightIntFunctor: Functor[EitherRightInt] =
    new Functor[EitherRightInt] {
      def map[A, B](fa: EitherRightInt[A])(f: A => B): EitherRightInt[B] =
        fa match {
          case Left(v)  => Left(v)
  ...
  val eitherLeftIntFunctor: Functor[EitherLeftInt] =
    new Functor[EitherLeftInt] {
      def map[A, B](fa: EitherLeftInt[A])(f: A => B): EitherLeftInt[B] =
        fa match {
