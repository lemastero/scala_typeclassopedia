...
object Yoneda {
  ...
  trait Yoneda[F[_], A] { self =>
    ...
    def yonedaToRan: Ran[Id, F, A] =
      new Ran[Id, F, A] {
        def runRan[B](f: A => Id[B]): F[B] = run(a => f(a))
      }

    def hoistYoneda[G[_]](fg: F ~> G): Yoneda[G, A] =
      new Yoneda[G, A] {
  ...
  object Yoneda {
    ...
    def liftYoneda[F[_], A](
        fa: F[A]
    )(implicit FunctorF: Functor[F]): Yoneda[F, A] =
      new Yoneda[F, A] {
        def run[R2](f: A => R2): F[R2] = FunctorF.map(fa)(f)
      }

    def ranToYoneda[F[_], A](r: Ran[Id, F, A]): Yoneda[F, A] =
      new Yoneda[F, A] {
