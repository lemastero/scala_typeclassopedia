 ...
 */
final case class Stream[T](value: () => T, tail: () => Stream[T]) {
...
object Representable {
  ...
  val repFunction0: Representable[Function0, Unit] =
  ...
  val streamRep: Representable[Stream, Int] = new Representable[Stream, Int] {
    def tabulate[X](f: Int => X): Stream[X] =
      Stream(value = () => f(0), tail = () => tabulate(f compose (_ + 1)))

    def index[X](fx: Stream[X]): Int => X = fx.get
  }

  val pair2Rep: Representable[Pair, Boolean] =
    new Representable[Pair, Boolean] {
  ...
  val uniformPairRep: Representable[UniformPair, Boolean] =
    new Representable[UniformPair, Boolean] {
      def tabulate[X](f: Boolean => X): UniformPair[X] = (f(false), f(true))
      def index[X](fa: UniformPair[X]): Boolean => X =
        b => if (b) fa._2 else fa._1
    }

  type UniformTriple[P] = Tuple3[P, P, P]
  val uniformTripleRep: Representable[UniformTriple, Option[Boolean]] =
    new Representable[UniformTriple, Option[Boolean]] {
      def tabulate[X](f: Option[Boolean] => X): UniformTriple[X] =
        (f(None), f(Some(false)), f(Some(true)))
      def index[X](fa: UniformTriple[X]): Option[Boolean] => X =
        b => b.map { rawB => if (rawB) fa._3 else fa._2 }.getOrElse(fa._1)
    }
}
...
trait Representable[F[_], RepF] { self =>
  def tabulate[X](
      f: RepF => X
  ): F[X] // memoization - convert function into Container
