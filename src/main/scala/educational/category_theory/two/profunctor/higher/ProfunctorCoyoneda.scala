...
object ProfunctorCoyoneda {
  ...
  def apply[XX, YY, P[_, _], A, B](
      ax: A => XX,
      yb: YY => B,
      p: P[XX, YY]
  ): ProfunctorCoyoneda[P, A, B] =
    new ProfunctorCoyoneda[P, A, B] {
      type X = XX
      type Y = YY
      def f1: A => X = ax
      def f2: Y => B = yb
  ...
  /**
    ...
    */
  def joinCoyoneda[P[_, _], A, B](
      p: ProfunctorCoyoneda[ProfunctorCoyoneda[P, *, *], A, B]
  ): ProfunctorCoyoneda[P, A, B] = ???
}

object ProfunctorCoyonedaInstances {
  def profunctor[P[_, _]]: Profunctor[ProfunctorCoyoneda[P, *, *]] =
    new Profunctor[ProfunctorCoyoneda[P, *, *]] {
      def dimap[X, W, Y, Z](
          p: ProfunctorCoyoneda[P, Y, Z]
      )(l: X => Y, r: Z => W): ProfunctorCoyoneda[P, X, W] =
        p.dimap(l, r)
    }
}
