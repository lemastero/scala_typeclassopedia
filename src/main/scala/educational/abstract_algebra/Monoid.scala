...
object MonoidInstances {
  def createMonoid[A](mempty: A, mapply: (A, A) => A): Monoid[A] =
    new Monoid[A] {
      def empty: A = mempty
  ...
  implicit def optionFirstNonEmpty[A]: Monoid[Option[A]] =
    createMonoid(None, _ orElse _)

  implicit val maxLongMonoid: Monoid[Long] =
    createMonoid(Long.MinValue, Math.max)
  ...
  implicit def endoFunction[A]: Monoid[A => A] =
    createMonoid(identity[A], _ andThen _)

  /**
    * Create Monoid for tuple from two Monoids
    */
  implicit def createMonoidTuple[L, R](
      left: Monoid[L],
      right: Monoid[R]
  ): Monoid[(L, R)] =
    new Monoid[(L, R)] {
