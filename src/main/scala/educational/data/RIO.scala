...
object RIOInstances {
  def rioFunctor[R, E]: Functor[RIO[R, *]] =
    new Functor[RIO[R, *]] {
  ...
  def rioContravariant[A]: Contravariant[RIO[*, A]] =
    new Contravariant[RIO[*, A]] {
      def contramap[R, RR](fa: RIO[R, A])(f: RR => R): RIO[RR, A] =
        RIO { rr => fa.run(f(rr)) }
    }

  def rioDivide[F[_], A](implicit MA: Monoid[A]): Divide[RIO[*, A]] =
    new Divide[RIO[*, A]] {
      def divide[RR, B, C](
          f: RR => (B, C),
          fb: RIO[B, A],
  ...
  def rioProfunctor[E]: Profunctor[RIO[*, *]] =
    new Profunctor[RIO[*, *]] {
      override def dimap[S, T, A, B](
          pab: RIO[A, B]
      )(ab: S => A, cd: B => T): RIO[S, T] =
        RIO((ab andThen pab.run) andThen cd)
    }
}
