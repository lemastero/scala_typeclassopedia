...
object TRIOInstances {
  def trioFunctor[R, E]: Functor[TRIO[R, E, *]] =
    new Functor[TRIO[R, E, *]] {
  ...
  trait TRIOMonad[R, E] extends Monad[TRIO[R, E, *]] {
    ...
    def flatMap[A, B](fa: TRIO[R, E, A])(f: A => TRIO[R, E, B]): TRIO[R, E, B] =
      fa.flatMap(f)
  }

  def trioMonad[R, E]: Monad[TRIO[R, E, *]] = new TRIOMonad[R, E] {}

  def trioContravariant[E, A]: Contravariant[TRIO[*, E, A]] =
    new Contravariant[TRIO[*, E, A]] {
      def contramap[R, RR](fa: TRIO[R, E, A])(f: RR => R): TRIO[RR, E, A] =
        TRIO { rr => fa.run(f(rr)) }
    }

  def trioDivide[F[_], E, A](implicit
      ME: Monoid[E],
      MA: Monoid[A]
  ): Divide[TRIO[*, E, A]] =
    new Divide[TRIO[*, E, A]] {
      ...
      override def contramap[R, RR](
          fa: TRIO[R, E, A]
      )(f: RR => R): TRIO[RR, E, A] =
        TRIO { rr => fa.run(f(rr)) }
    }

  def trioProfunctor[E]: Profunctor[TRIO[*, E, *]] =
    new Profunctor[TRIO[*, E, *]] {
      def dimap[RR, AA, R, A](
          fa: TRIO[R, E, A]
      )(f: RR => R, g: A => AA): TRIO[RR, E, AA] =
        TRIO { rr => fa.map(g).run(f(rr)) }
    }

  def trioBifunctor[R]: Bifunctor[TRIO[R, *, *]] =
    new Bifunctor[TRIO[R, *, *]] {
      override def bimap[E, EE, A, AA](
          fa: TRIO[R, E, A]
      )(f: E => EE, g: A => AA): TRIO[R, EE, AA] =
        TRIO { rr =>
          fa.run(rr) match {
