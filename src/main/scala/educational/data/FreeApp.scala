...
object Value {
  def apply[F[_], B, AA](faa: F[AA], faab: FreeAp[F, AA => B]): Value[F, B] =
    new Value[F, B] {
      type A = AA
      val fa: F[AA] = faa
...
object FreeApInstanes {
  def freeAppApplicative[F[_]]: Applicative[FreeAp[F, *]] =
    new Applicative[FreeAp[F, *]] {
      override def map[X, Y](fa: FreeAp[F, X])(f: X => Y): FreeAp[F, Y] =
        fa match {
