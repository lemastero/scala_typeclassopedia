...
object Reader {
  ...
  def readerMonad[C]: Monad[Reader[C, *]] =
    new Monad[Reader[C, *]] {
      override def map[A, B](x: Reader[C, A])(f: A => B): Reader[C, B] =
        Reader(x.run andThen f)
  ...
  def readerContra[V]: Contravariant[Reader[*, V]] =
    new Contravariant[Reader[*, V]] {
      def contramap[A, B](fa: Reader[A, V])(f: B => A): Reader[B, V] =
        Reader(f andThen fa.run)
    }
}
