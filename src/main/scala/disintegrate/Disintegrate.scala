...
object Disintegrate {
  ...
  def disFunction[C]: Disintegrate[C => *] =
    new Disintegrate[C => *] {
  ...
  def disFunctor[F[_]](implicit FC: Functor[F]): Disintegrate[F] =
    new Disintegrate[F] {
  ...
  def distState[S]: Disintegrate[State[S, *]] =
    new Disintegrate[State[S, *]] {
      ...
      def disintegrate2[A, B](mab: State[S, (A, B)]): A => State[S, B] =
        _ => {
          State(b => {
            val (ab, b2) = mab.runState(b)
            (ab._2, b2)
...
trait DisintegrateMonoid[F[_]] {
  def disintegrate[A, B](
      fab: F[(A, B)]
  )(implicit MB: Monoid[B]): (F[A], A => F[B]) =
...
object DisintegrateMonoid {
  ...
  val disMonPredicate: DisintegrateMonoid[Predicate] =
    new DisintegrateMonoid[Predicate] {

      def disintegrate1[A, B](
          fab: Predicate[(A, B)]
      )(implicit MB: Monoid[B]): Predicate[A] =
  ...
  def disMonFun[C]: DisintegrateMonoid[* => C] =
    new DisintegrateMonoid[* => C] {

      def disintegrate1[A, B](
          fab: ((A, B)) => C
      )(implicit MB: Monoid[B]): A => C =
        a => fab((a, MB.empty))

      def disintegrate2[A, B](fab: ((A, B)) => C): A => B => C = { a => b =>
        fab((a, b))
      }
    }

  def disMonContra[F[_]](implicit C: Contravariant[F]): DisintegrateMonoid[F] =
    new DisintegrateMonoid[F] {
      def disintegrate1[A, B](fab: F[(A, B)])(implicit MB: Monoid[B]): F[A] =
...
trait DisintegrateE[F[_]] {
  def disintegrate[X, Y](fab: F[Either[X, Y]])(implicit
      MB: Monoid[Y]
  ): Either[F[X], X => F[Y]]
}
