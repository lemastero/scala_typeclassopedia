...
classDiagram
  ...
  class Bifoldable {
    <<P[+_,+_]>>
    C bifoldLeft(F[A,B], C, Tuple2[C,A] => C, Tuple2[C,B] => C)
  }
  class Bitraverse {
    <<P[+_,+_]>>
    G[F[C,D]] bitraverse[G: Applicative](F[A,B], A=>G[C], B => G[D])
  }
  class Biapply {
    <<P[+_,+_]>>
    F[AA,BB] biApply(F[A,B], F[A=>AA,B=>BB])
  }
  class Biapplicative {
    <<P[+_,+_]>>
    F[A,B] bipure(a: A, b: B)
  }
  class Zivariant {
    <<Z[-_,+_,+_]>>
